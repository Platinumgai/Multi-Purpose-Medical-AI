...
import { Send, User, Bot, Calendar, AlertCircle, CheckCircle } from 'lucide-react';
...
export function SymptomsChecker() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: "Hello! I'm your AI medical assistant. I'll help analyze your symptoms and recommend appropriate care. Please describe your symptoms.",
      isBot: true,
      timestamp: new Date()
    }
  ]);
  ...
  // Integrated features from friend's AddSymptom (non-invasive to SVG)
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedSymptoms, setSelectedSymptoms] = useState<string[]>([]);
  const [selectedSymptomsFromModel, setSelectedSymptomsFromModel] = useState<string[]>([]);
  ...
  const [hasBackView, setHasBackView] = useState<boolean>(false);
  ...
  // Expanded region → symptoms map (front + back). Falls back if unmapped.
  const regionSymptoms: Record<string, string[]> = {
    'front-head': ['Headache', 'Dizziness', 'Facial pain', 'Vision problems'],
    'front-neck': ['Neck pain', 'Sore throat', 'Swollen glands'],
    'front-chest': ['Chest Pain', 'Shortness of Breath', 'Palpitations', 'Cough'],
    'front-upper_arm': ['Upper arm pain', 'Numbness', 'Tingling'],
    'front-forearm': ['Forearm pain', 'Wrist pain', 'Numbness'],
    'front-upper_abdomen': ['Upper abdominal pain', 'Nausea', 'Heartburn'],
    'front-mid_abdomen': ['Stomach cramps', 'Bloating', 'Nausea'],
    'front-lower_abdomen': ['Lower abdominal pain', 'Pelvic pain', 'Cramps'],
    'front-shoulder': ['Shoulder pain', 'Limited mobility', 'Stiffness'],
    'front-hand': ['Hand pain', 'Finger numbness', 'Swelling'],
    'front-thigh': ['Thigh pain', 'Muscle strain', 'Bruising'],
    'front-knee': ['Knee pain', 'Swelling', 'Instability'],
    'front-shin': ['Shin pain', 'Tenderness', 'Swelling'],
    'front-foot': ['Foot pain', 'Ankle swelling', 'Toe numbness'],
    'front-groin': ['Groin pain', 'Swelling', 'Hernia-like bulge'],
    // Back regions
    'back-head': ['Headache (occipital)', 'Neck stiffness', 'Scalp tenderness'],
    'back-neck': ['Neck pain', 'Muscle spasm', 'Limited rotation'],
    'back-upper_back': ['Upper back pain', 'Muscle tightness', 'Burning sensation'],
    'back-mid_back': ['Mid-back pain', 'Rib pain', 'Stiffness'],
    'back-lower_back': ['Lower back pain', 'Sciatica', 'Stiffness'],
    'back-shoulder': ['Shoulder blade pain', 'Radiating pain', 'Muscle knot'],
    'back-upper_arm': ['Upper arm pain', 'Tingling', 'Weakness'],
    'back-forearm': ['Forearm pain', 'Numbness', 'Tingling'],
    'back-hand': ['Hand pain', 'Finger numbness', 'Weak grip'],
    'back-buttock': ['Buttock pain', 'Piriformis pain', 'Sciatica'],
    'back-thigh': ['Hamstring pain', 'Cramp', 'Bruising'],
    'back-knee': ['Knee pain (posterior)', 'Swelling', "Baker's cyst"],
    'back-calf': ['Calf pain', 'Cramp', 'Tenderness'],
    'back-foot': ['Heel pain', 'Plantar fasciitis', 'Ankle pain'],
  };

  // Gender overlays for certain regions without changing the model
  const femaleRegionOverlay: Record<string, string[]> = {
    'front-chest': ['Breast tenderness', 'Breast lump', 'Nipple discharge'],
    'front-lower_abdomen': ['Menstrual cramps', 'Pelvic pressure', 'Irregular bleeding'],
    'front-groin': ['Pelvic pain', 'Dyspareunia'],
  };
  const maleRegionOverlay: Record<string, string[]> = {
    'front-groin': ['Testicular pain', 'Inguinal discomfort'],
    'front-lower_abdomen': ['Prostatitis-like discomfort'],
  };

  const getSymptomsForRegion = (id: string): string[] => {
    const base = regionSymptoms[id] || [];
    const overlay = gender === 'female' ? femaleRegionOverlay[id] || [] : maleRegionOverlay[id] || [];
    const combined = [...base, ...overlay];
    return combined.length > 0 ? combined : defaultRegionSymptoms;
  };

  const defaultRegionSymptoms = useMemo(
    () => ['Pain', 'Swelling', 'Numbness', 'Redness', 'Itching', 'Stiffness'],
    []
  );

  // Detect if back-side paths exist to guard rotate toggle
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;
    const backNodes = svg.querySelector('[id^="back-"]');
    setHasBackView(!!backNodes);
  }, []);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const responses = [
        "I understand you're experiencing these symptoms. How long have you been feeling this way?",
        ...
        "Based on your symptoms, I'm analyzing possible conditions. Please wait..."
      ];
  ...
  const filteredSuggestions = searchQuery
    ? symptoms.filter(s => s.toLowerCase().includes(searchQuery.toLowerCase()) && !selectedSymptoms.includes(s))
    : [];
  ...
  const removeSymptomFromSearch = (symptom: string) => {
    setSelectedSymptoms(prev => prev.filter(s => s !== symptom));
  };

  const addSymptomFromRegion = (symptom: string) => {
    if (!selectedSymptomsFromModel.includes(symptom)) {
      setSelectedSymptomsFromModel(prev => [...prev, symptom]);
    }
  };

  const removeSymptomFromRegion = (symptom: string) => {
    setSelectedSymptomsFromModel(prev => prev.filter(s => s !== symptom));
  };

  // Delegate SVG path clicks to open region modal
  const onSvgClick: React.MouseEventHandler<SVGSVGElement> = (e) => {
    ...
    if (tag === 'path' || tag === 'g') {
      ...
      if (!id) return;
      // Toggle highlight via state; effect will paint fills
      setHighlightedRegions(prev => {
        const next = new Set(prev);
        if (next.has(id)) next.delete(id); else next.add(id);
        return next;
      });
      // Always open modal; use fallback if unmapped
      setActiveRegionId(id);
      setIsRegionModalOpen(true);
    }
  };

  // Rotate effect: show front-* or back-* regions by toggling display styles
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;
    const showPrefix = isFrontView ? 'front-' : 'back-';
    const hidePrefix = isFrontView ? 'back-' : 'front-';
    const allIds = svg.querySelectorAll('[id]');
    allIds.forEach((node) => {
      const id = (node as Element).getAttribute('id') || '';
      if (!id) return;
      if (id.startsWith(showPrefix)) {
        (node as SVGElement).style.display = '';
      } else if (id.startsWith(hidePrefix)) {
        (node as SVGElement).style.display = 'none';
      }
    });
  }, [isFrontView]);

  // Hover effect: temporarily highlight hovered region (without toggling selection)
  useEffect(() => {
    const svg = svgRef.current;
    if (!svg) return;

    // Clear all prior hover highlights first
    const all = svg.querySelectorAll('[id]');
    all.forEach((n) => {
      const path = n as SVGPathElement;
      const id = path.getAttribute('id');
      if (!id) return;
      if (path.dataset.hovering === 'true') {
        // Restore only if not selected
        if (!highlightedRegions.has(id)) {
          if (path.dataset.origFill !== undefined) {
            path.style.fill = path.dataset.origFill;
          } else {
            path.style.fill = '';
          }
        }
        path.dataset.hovering = 'false';
      }
    });

    if (!hoverRegionId) return;
    const node = document.getElementById(hoverRegionId) as SVGPathElement | null;
    if (!node) return;
    node.dataset.hovering = 'true';
    if (!node.dataset.origFill) node.dataset.origFill = node.style.fill || '';
    // Only apply hover color if not already selected
    if (!highlightedRegions.has(hoverRegionId)) {
      node.style.fill = '#3399ff';
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hoverRegionId, highlightedRegions]);

  const onSvgMouseOver: React.MouseEventHandler<SVGSVGElement> = (e) => {
    const el = e.target as HTMLElement | null;
    if (!el) return;
    const id = el.getAttribute('id');
    if (id) setHoverRegionId(id);
  };
  ...
  const sendSelectionsToChat = () => {
    const combined = [...selectedSymptoms, ...selectedSymptomsFromModel];
    if (combined.length === 0) return;
    const text = `Symptoms selected: ${combined.join(', ')}`;
    const newMessage: Message = {
      id: messages.length + 1,
      text,
      isBot: false,
    ...
    setMessages(prev => [...prev, newMessage]);
    // Auto bot response acknowledging symptoms
    const botMessage: Message = {
      id: messages.length + 2,
      text: "I understand you're experiencing these symptoms. How long have you been feeling this way?",
      isBot: true,
      timestamp: new Date()
    };
    setTimeout(() => {
  ...
  // Apply highlight fill to selected regions without altering original SVG markup
  useEffect(() => {
    const highlightColor = '#5cc2ff';
    highlightedRegions.forEach((id) => {
      const node = document.getElementById(id) as SVGPathElement | null;
      if (!node) return;
      if (!node.dataset.origFill) {
        node.dataset.origFill = node.getAttribute('fill') || '';
      }
      node.setAttribute('fill', highlightColor);
    });
    // Revert non-highlighted nodes to original fill
    // Scan existing paths with id to cover mapped and unmapped regions
    document.querySelectorAll('svg path[id], svg g[id]').forEach((n) => {
      const id = (n as HTMLElement).getAttribute('id');
      if (!id) return;
      if (!highlightedRegions.has(id)) {
        const path = n as SVGPathElement;
        if (path.dataset.origFill !== undefined) {
          path.setAttribute('fill', path.dataset.origFill || '#c2e0f8');
        }
      }
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const selectSymptom = (symptom: string) => {
    const newMessage: Message = {
      id: messages.length + 1,
      text: `I'm experiencing ${symptom.toLowerCase()}`,
      isBot: false,
      timestamp: new Date()
    };
    ...
    setTimeout(() => {
      const botResponse: Message = {
        id: messages.length + 2,
        text: `I see you're experiencing ${symptom.toLowerCase()}. Can you tell me more about when this started and how severe it is?`,
        isBot: true,
        timestamp: new Date()
      };
      setMessages(prev => [...prev, botResponse]);
  ...
  return (
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600">
            Get instant AI analysis of your symptoms and smart recommendations
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" ref={chatRef}>
          <div className="lg:col-span-2 ">
            {/* Search + selections panel (integrated features) */}
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 mb-4 p-4">
              <div className="flex items-center gap-2">
                ...
                <button
                  className={`px-3 py-2 rounded-lg text-sm ${hasBackView ? 'bg-gray-100' : 'bg-gray-100 opacity-50 cursor-not-allowed'}`}
                  onClick={() => hasBackView && setIsFrontView(v => !v)}
                  title={hasBackView ? 'Rotate model' : 'Back view not available'}
                >
                ...
                <div className="mt-2 border border-gray-200 rounded-lg max-h-40 overflow-auto">
                  {filteredSuggestions.map(s => (
                    <button
                      ...
                      onClick={() => addSymptomFromSearch(s)}
                    >
                      {s}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
                  <h4 className="font-medium text-gray-900 mb-2 text-sm">Selected Symptoms</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedSymptoms.map(s => (
                      <span key={s} className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-1 rounded-full text-xs">
                        {s}
                        <button onClick={() => removeSymptomFromSearch(s)} className="ml-1">×</button>
                      </span>
                    ))}
                ...
                <div>
                  <h4 className="font-medium text-gray-900 mb-2 text-sm">Selected From Model</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedSymptomsFromModel.map(s => (
                      <span key={s} className="inline-flex items-center gap-1 bg-teal-50 text-teal-700 px-2 py-1 rounded-full text-xs">
                        {s}
                        <button onClick={() => removeSymptomFromRegion(s)} className="ml-1">×</button>
                      </span>
                    ))}
                    {selectedSymptomsFromModel.length === 0 && (
                      <span className="text-xs text-gray-500">None selected</span>
                    )}
                  </div>
                </div>
              ...
              <input type="hidden" name="selected_symptoms" value={JSON.stringify(selectedSymptoms)} />
              <input type="hidden" name="selected_symptoms_frommodel" value={JSON.stringify(selectedSymptomsFromModel)} />
              <div className="mt-4 flex justify-end">
                <button
                  type="button"
                  onClick={sendSelectionsToChat}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm disabled:opacity-50"
                  disabled={selectedSymptoms.length + selectedSymptomsFromModel.length === 0}
                >
                  Discuss in AI Chat
                </button>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 h-[600px] flex flex-col items-center">
              
              <svg ref={svgRef} width="240" height="480" viewBox="0 -60 150 400" fill="none" xmlns="https://www.w3.org/TR/SVG2/" onClick={onSvgClick} onMouseOver={onSvgMouseOver} onMouseLeave={onSvgMouseLeave}>
                <g style={{ display: isFrontView ? 'block' : 'none' }}>
                  <path
                    d="M72.129 34.841l-.085-.418v.007l-.39-3.008-.061.003a2.406 2.406 0 0 1-.723-.114 3.174 3.174 0 0 1-.72-.325c-.229-.14-.456-.317-.602-.547a3.225 3.225 0 0 1-.295-.669 17.65 17.65 0 0 1-.342-1.128 38.729 38.729 0 0 1-.714-3.207 4.602 4.602 0 0 1-.06-.453c-.019-.277-.039-.554-.05-.831a9.72 9.72 0 0 1 .015-1.017 6.224 6.224 0 0 1 .068-.528c.073-.385.212-.792.485-1.079-.026-.256-.048-.514-.07-.771a106.275 106.275 0 0 1-.277-4.219 66.03 66.03 0 0 1-.075-3.36 33.3 33.3 0 0 1 .105-2.643c.065-.747.16-1.499.373-2.22.074-.236.154-.471.234-.705.219-.633.452-1.266.762-1.861.291-.558.661-1.079 1.162-1.468.32-.249.681-.436 1.061-.575.511-.188 1.045-.291 1.582-.376l.079-.052c1.08-.698 2.246-1.266 3.443-1.736A21.897 21.897 0 0 1 82.235.203a21.556 21.556 0 0 1 2.858-.168c1.713.026 3.432.241 5.081.716 1.213.348 2.387.845 3.46 1.513a11.474 11.474 0 0 1 2.218 1.796c.915.958 1.653 2.08 2.169 3.299.201.475.37.965.497 1.466.079.308.145.621.181.938.028.25.016.5.005.752-.014.323-.033.647-.053.969a300.61 300.61 0 0 1-.194 2.839c-.184 2.526-.384 5.051-.584 7.576l-.032.397a3.558 3.558 0 0 1 .12.61 6.96 6.96 0 0 1 .046.824c0 .184-.005.367-.013.551-.013.248-.03.495-.048.742l-.017.158a22.855 22.855 0 0 1-.328 1.716 33.86 33.86 0 0 1-.539 2.153c-.085.296-.175.59-.284.877a2.438 2.438 0 0 1-.277.569 1.69 1.69 0 0 1-.403.384 3.132 3.132 0 0 1-1.072.482c-.15.035-.31.06-.469.063-.144 1.407-.279 2.662-.381 3.203l-.103.092-.029-.297c-.001.051-.003.1-.007.151-.007.074-.007.074-.017.148l-.018.062-.325.29c-1.077.873-1.976 1.641-3.244 2.199-1.903.839-4.287 1.45-6.361 1.575-1.231.073-2.429-.024-3.649-.211-2.173-.335-4.379-.892-6.281-1.991-.744-.431-1.363-1.263-2.013-1.805z"
                ...
                <g style={{ display: isFrontView ? 'none' : 'block' }}>
                  ...
                  />
                  {/* Upper arms */}
                  <rect id="back-upper_arm" className="sc-body-model-svg__path" x="35" y="45" width="10" height="40" rx="5" fill="#c2e0f8" />
                  <rect id="back-upper_arm_r" className="sc-body-model-svg__path" x="102" y="45" width="10" height="40" rx="5" fill="#c2e0f8" />
                  {/* Forearms */}
                  <rect id="back-forearm" className="sc-body-model-svg__path" x="33" y="85" width="12" height="38" rx="5" fill="#c2e0f8" />
                  <rect id="back-forearm_r" className="sc-body-model-svg__path" x="102" y="85" width="12" height="38" rx="5" fill="#c2e0f8" />
                  {/* Upper abdomen / mid back */}
                  <rect id="back-upper_abdomen" className="sc-body-model-svg__path" x="55" y="68" width="40" height="26" rx="6" fill="#c2e0f8" />
                  <rect id="back-mid_abdomen" className="sc-body-model-svg__path" x="55" y="94" width="40" height="26" rx="6" fill="#c2e0f8" />
                  {/* Lower back / pelvis */}
                  <rect id="back-lower_abdomen" className="sc-body-model-svg__path" x="54" y="120" width="42" height="22" rx="6" fill="#c2e0f8" />
                  {/* Thighs */}
                  <rect id="back-thigh" className="sc-body-model-svg__path" x="58" y="142" width="14" height="40" rx="7" fill="#c2e0f8" />
                  <rect id="back-thigh_r" className="sc-body-model-svg__path" x="78" y="142" width="14" height="40" rx="7" fill="#c2e0f8" />
                  {/* Legs */}
                  <rect id="back-leg" className="sc-body-model-svg__path" x="58" y="182" width="14" height="50" rx="7" fill="#c2e0f8" />
                  <rect id="back-leg_r" className="sc-body-model-svg__path" x="78" y="182" width="14" height="50" rx="7" fill="#c2e0f8" />
                </g>

              </svg>
              <p className="text-sm text-gray-500 mt-2 text-center">
                Select a symptom or describe where you feel discomfort.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              <h3 className="font-semibold text-gray-900 mb-4">Common Symptoms</h3>
              <div className="grid grid-cols-2 gap-2">
                {symptoms.map((symptom) => (
                  <button
                    key={symptom}
                    onClick={() => selectSymptom(symptom)}
                    className="text-sm px-3 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors"
                  >
                    {symptom}
                  </button>
                ))}
            ...
            {showRecommendation && (
              <div className="bg-green-50 rounded-2xl border border-green-200 p-6">
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h3 className="font-semibold text-green-900">AI Recommendation</h3>
                </div>
                <div className="space-y-3">
                  <p className="text-green-800">Based on your symptoms, I recommend:</p>
                  <ul className="space-y-2 text-sm text-green-700">
                    <li>• Consult with a General Physician</li>
                    <li>• Consider basic rest and hydration</li>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 h-[600px] flex flex-col">
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-4">
                {messages.map((message) => (
                  ...
                  >
                    <div className={`flex items-start space-x-3 max-w-md ${message.isBot ? '' : 'flex-row-reverse space-x-reverse'}`}>
                      ...
                      <div className={`px-4 py-2 rounded-2xl ${message.isBot
    ...
    {isRegionModalOpen && !!activeRegionId && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div className="bg-white w-full max-w-md rounded-2xl shadow-lg p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-900 text-sm">Select symptoms for: {activeRegionId}</h3>
            <button onClick={closeRegionModal} className="text-gray-500 hover:text-gray-700">×</button>
          </div>
          <div className="space-y-2 max-h-64 overflow-auto">
            {getSymptomsForRegion(activeRegionId!).map((item: string) => (
              <button
                key={item}
                className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 border border-gray-200"
                onClick={() => addSymptomFromRegion(item)}
              >
                {item}
              </button>
            ))}
          ...
          <div className="mt-4 text-right">
            <button onClick={closeRegionModal} className="px-4 py-2 bg-blue-600 text-white rounded-lg">Done</button>
          </div>
        </div>
      </div>
    )}
